...
export default function StyleProfileSelector({
  ...
  const handleExportProfile = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const path = await files.selectJsonOutput(`${profileExport.profile.name}.json`)
      if (path) {
        await files.writeTextFile(path, JSON.stringify(profileExport, null, 2))
  ...
  const handleImportProfile = useCallback(async () => {
    setIsLoading(true)
    try {
      const path = await files.selectJsonInput()
      if (path) {
        const contents = await files.readTextFile(path)
        const profileExport = JSON.parse(contents)
        const { profile: imported, validation } = importProfile(profileExport)
        if (imported) {
